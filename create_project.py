...
def copy_data(path, ds_name, name, invert=False):
...
def add_data(path, ds_name, name, menu, invert=False):
    tmp_path, resolution, vrange = copy_data(path, ds_name, name, invert=invert)
...
def get_organelles(name, cell_id):
    ...
    for path in all_paths:
        ...
        if name.startswith('Cell-coccolith'):
            organelle_paths.append(path)
            organelle_names.append('coccolith')
...
def create_split_dataset(name):
    ...
    for cell_id in (1, 2, 3):
        ...
        add_data(seg_path, ds_name, seg_name, menu='segmentation')
        ...
        print(organelle_names)
        seg_name = f'cell{cell_id}-organelles'
        add_segmentations(organelle_paths, organelle_names, ds_name, seg_name, menu='segmentation')
...
def add_merged_cell_mask(name, ds_name):
    root_ds = os.path.join(root, name)
    data = None
    for cell_id in (1, 2, 3):
        path = os.path.join(root_ds, f'Data{cell_id}/Segmented-files/Cell{cell_id}.nrrd')
    ...
    tmp_path = f'./tmp_data/{name}/merged_cell_masks.n5'
    with z5py.File(tmp_path, 'w') as f:
        f.create_dataset('data', data=data, compression='gzip', chunks=chunks, n_threads=8)
    mobie.add_segmentation(tmp_path, 'data', './data', ds_name,
                           segmentation_name="cell-mask", resolution=resolution,
                           scale_factors=scale_factors, chunks=chunks,
...
# TODO add data for the remote s3 store and upload it
